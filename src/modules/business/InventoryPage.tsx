...
export function InventoryPage() {
  ...
  async function chooseFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const existingCodes = new Set(products.map((product) => product.code.trim().toLocaleLowerCase('es')))
      const existingDuplicates = result.products.filter((product) => existingCodes.has(product.code.trim().toLocaleLowerCase('es'))).map((product) => product.code)
      setPreview(existingDuplicates.length ? { ...result, errors: [...result.errors, ...existingDuplicates.map((code) => ({ row: 0, code, message: `El código ${code} ya existe en el inventario.` }))] } : result)
