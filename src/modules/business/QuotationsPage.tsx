import { CalendarCheck, Download, FileSpreadsheet, FileText, Mail, MessageCircle, Pencil, Plus, ReceiptText, Search, ShoppingCart, Trash2 } from 'lucide-react'
...
import './QuotationsShortages.scss'
...
export function QuotationsPage() {
  ...
  const [exportingShortages, setExportingShortages] = useState(false)
  useEffect(() => { if (!businessId) return; setInventoryReady(false); void getInventoryProducts(businessId).then((inventory) => { setProducts(inventory); setInventoryReady(true) }).catch(() => { setProducts([]); toast.error('No fue posible cargar el inventario', 'Reintenta antes de generar el reporte de faltantes.') }) }, [businessId, toast])
  useEffect(() => { if (drawerOpen) navigate('/app/cotizaciones/nueva') }, [drawerOpen, navigate])
  ...
  const selectableFiltered = filtered.filter((quote) => quote.status !== 'converted')
  const allFilteredSelected = selectableFiltered.length > 0 && selectableFiltered.every((quote) => selectedQuotationIds.has(quote.id))

  function toggleQuotation(quotationId: string) {
    setSelectedQuotationIds((current) => {
      const next = new Set(current)
      if (next.has(quotationId)) next.delete(quotationId); else next.add(quotationId)
      return next
    })
  }
  ...
  function toggleAllFiltered() {
    setSelectedQuotationIds((current) => {
      const next = new Set(current)
      if (allFilteredSelected) selectableFiltered.forEach((quote) => next.delete(quote.id)); else selectableFiltered.forEach((quote) => next.add(quote.id))
      return next
    })
  }

  async function exportShortages() {
    const selected = quotations.filter((quotation) => selectedQuotationIds.has(quotation.id) && quotation.status !== 'converted')
    if (!selected.length) return toast.warning('Selecciona cotizaciones', 'Marca al menos una cotización pendiente para generar el reporte.')
    if (!inventoryReady) return toast.warning('Inventario no disponible', 'Espera a que cargue el inventario o recarga la página antes de generar el reporte.')
    try {
    ...
    finally { setExportingShortages(false) }
  }
  ...
  const columns: DataTableColumn<Quotation>[] = [
    { key: 'select', header: '', width: '52px', align: 'center', render: (quote) => <input className="quotation-checkbox" type="checkbox" aria-label={`Seleccionar ${quote.number}`} checked={selectedQuotationIds.has(quote.id)} disabled={quote.status === 'converted'} title={quote.status === 'converted' ? 'La cotización ya fue convertida en venta.' : `Seleccionar ${quote.number}`} onChange={() => toggleQuotation(quote.id)} /> },
    ...
    { key: 'actions', header: '', align: 'right', render: (quote) => <div className="quotation-actions">{quote.status !== 'converted' && <IconButton icon={<Pencil size={16} />} label={`Editar ${quote.number}`} onClick={() => navigate(`/app/cotizaciones/${encodeURIComponent(quote.id)}/editar`)} />}{quote.status !== 'converted' && <IconButton icon={<ShoppingCart size={16} />} label={`Convertir ${quote.number} en venta`} onClick={() => navigate(`/app/pos?quotationId=${encodeURIComponent(quote.id)}`)} />}<IconButton icon={<Download size={16} />} label={`Descargar ${quote.number}`} disabled={preparingId === quote.id} onClick={() => void download(quote)} /><IconButton icon={<Mail size={16} />} label={`Enviar ${quote.number} por correo`} disabled={preparingId === quote.id} onClick={() => void shareEmail(quote)} /><IconButton icon={<MessageCircle size={16} />} label={`Enviar ${quote.number} por WhatsApp`} disabled={preparingId === quote.id} onClick={() => void shareWhatsApp(quote)} />{quote.status !== 'converted' && <IconButton icon={<Trash2 size={16} />} label={`Eliminar ${quote.number}`} variant="danger" onClick={() => setQuotationToDelete(quote)} />}<ConfirmDialog open={quotationToDelete?.id === quote.id} title="Eliminar cotización" description={`Se eliminará ${quote.number}. Esta acción no afecta el inventario.`} confirmText="Eliminar" loading={deleting} onClose={() => setQuotationToDelete(null)} onConfirm={() => void remove()} /></div> },
  ]
  return <div className="quotations-page"><header className="quotations-page__header"><div><span>Operación</span><h2>Cotizaciones</h2><p>Prepara propuestas con precios de venta e IVA del 15%.</p></div><div className="quotations-page__header-actions"><Button variant="secondary" icon={<FileSpreadsheet size={18} />} loading={exportingShortages} disabled={!selectedQuotationIds.size} onClick={() => void exportShortages()}>Reporte de faltantes{selectedQuotationIds.size ? ` (${selectedQuotationIds.size})` : ''}</Button><Button icon={<Plus size={18} />} onClick={() => setDrawerOpen(true)}>Nueva cotización</Button></div></header><section className="quotation-stats"><Card><ReceiptText size={22} /><div><small>Cotizaciones</small><strong>{quotations.length}</strong></div></Card><Card><CalendarCheck size={22} /><div><small>Vigentes</small><strong>{active}</strong></div></Card><Card><FileText size={22} /><div><small>Valor cotizado</small><strong>{formatMoney(totalQuoted)}</strong></div></Card></section><section className="quotations-panel"><div className="quotations-selection"><label><input type="checkbox" checked={allFilteredSelected} disabled={!selectableFiltered.length} onChange={toggleAllFiltered} /> Seleccionar cotizaciones visibles</label><span>{selectedQuotationIds.size ? `${selectedQuotationIds.size} seleccionada${selectedQuotationIds.size === 1 ? '' : 's'}` : 'Selecciona una o varias para calcular faltantes'}</span></div><div className="quotations-toolbar"><div><Search size={18} /><input type="search" value={search} placeholder="Buscar por número, cliente o correo..." onChange={(event) => setSearch(event.target.value)} /></div><span>Los precios quedan guardados como fueron cotizados.</span></div>{error ? <div className="quotations-error"><p>{error}</p><Button variant="secondary" onClick={() => void reload()}>Reintentar</Button></div> : <DataTable columns={columns} data={filtered} getRowKey={(quote) => quote.id} loading={loading} emptyTitle="No hay cotizaciones" emptyDescription="Crea la primera cotización utilizando los productos del inventario." />}</section><QuoteDrawer open={drawerOpen} businessId={businessId} currency={currency} products={products} onClose={() => setDrawerOpen(false)} onSaved={() => void reload()} /></div>
}
